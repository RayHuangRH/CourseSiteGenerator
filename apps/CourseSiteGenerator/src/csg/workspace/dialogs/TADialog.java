...
public class TADialog extends Stage {
    ...
    private void initDialog() {
        ...
        GridPane gridPane = guiBuilder.buildGridPane(OH_TA_DIALOG_GRID_PANE, null, CLASS_CSG_DIALOG_GRID_PANE, ENABLED);
        guiBuilder.buildLabel(OH_TA_DIALOG_HEADER_LABEL, gridPane, 0, 0, 3, 1, CLASS_CSG_DIALOG_HEADER, ENABLED);
        guiBuilder.buildLabel(OH_TA_DIALOG_NAME_LABEL, gridPane, 0, 1, 1, 1, CLASS_CSG_DIALOG_PROMPT, ENABLED);
        TextField nameTextField = guiBuilder.buildTextField(OH_TA_DIALOG_NAME_TEXT_FIELD, gridPane, 1, 1, 2, 1, CLASS_CSG_DIALOG_TEXT_FIELD, ENABLED);
        guiBuilder.buildLabel(OH_TA_DIALOG_EMAIL_LABEL, gridPane, 0, 2, 1, 1, CLASS_CSG_DIALOG_PROMPT, ENABLED);
        TextField emailTextField = guiBuilder.buildTextField(OH_TA_DIALOG_EMAIL_TEXT_FIELD, gridPane, 1, 2, 2, 1, CLASS_CSG_DIALOG_TEXT_FIELD, ENABLED);
        guiBuilder.buildLabel(OH_TA_DIALOG_TYPE_LABEL, gridPane, 0, 3, 1, 1, CLASS_CSG_DIALOG_PROMPT, ENABLED);
        ToggleGroup tg = new ToggleGroup();
        RadioButton gradRadioButton = guiBuilder.buildRadioButton(OH_TA_DIALOG_GRAD_RADIO_BUTTON, gridPane, 1, 3, 1, 1, CLASS_CSG_DIALOG_RADIO_BUTTON, ENABLED, tg, true);
        RadioButton undergradRadioButton = guiBuilder.buildRadioButton(OH_TA_DIALOG_UNDERGRAD_RADIO_BUTTON, gridPane, 2, 3, 1, 1, CLASS_CSG_DIALOG_RADIO_BUTTON, ENABLED, tg, false);
        HBox okCancelBox = guiBuilder.buildHBox(OH_TA_DIALOG_OK_BOX, gridPane, 0, 4, 3, 1, CLASS_CSG_DIALOG_BOX, ENABLED);
        okCancelBox.setAlignment(Pos.CENTER);
        Button okButton = guiBuilder.buildTextButton(OH_TA_DIALOG_OK_BUTTON, okCancelBox, CLASS_CSG_DIALOG_BUTTON, !ENABLED);
        Button cancelButton = guiBuilder.buildTextButton(OH_TA_DIALOG_CANCEL_BUTTON, okCancelBox, CLASS_CSG_DIALOG_BUTTON, ENABLED);
    ...
    private void processCompleteWork() {
		...
		CourseSiteGeneratorData data = (CourseSiteGeneratorData)app.getDataComponent();
        EditTA_Transaction transaction = new EditTA_Transaction(data, taToEdit, name, email, type.toString());
