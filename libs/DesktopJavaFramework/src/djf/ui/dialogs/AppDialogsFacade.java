...
public class AppDialogsFacade {
	...
	public static void showExportDialog(AppTemplate app, String exportPath) throws IOException {
		AppWebDialog dialog = new AppWebDialog(app);
		if (new File(exportPath + "/index.html").exists()) {
			dialog.showWebDialog(exportPath + "/index.html");
		} else if (new File(exportPath + "/syllabus.html").exists()) {
			dialog.showWebDialog(exportPath + "/syllabus.html");
		} else if (new File(exportPath + "/schedule.html").exists()) {
			dialog.showWebDialog(exportPath + "/schedule.html");
		} else {
			dialog.showWebDialog(exportPath + "/hws.html");
		}
	}
	...
	public static File showImageDialog(Stage window, AppPropertyType openTitleProp) {
		FileChooser fc = new FileChooser();
		fc.setInitialDirectory(new File(PATH_ICONS));
		PropertiesManager props = PropertiesManager.getPropertiesManager();
